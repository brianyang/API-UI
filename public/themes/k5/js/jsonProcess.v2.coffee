...
postNewProject = ->
  ...
  featured = mainForm[5].value || 0
  console.log featured
  sticky = mainForm[6].value || 0
  console.log sticky
  labs = mainForm[7].value || 0
  console.log labs
    ...
    success: (d) ->
      ...
      createTagsObj = -># {{{
        ...
        $.each tagArr, (i,l) ->
          ...
          url = '/v1/tags/' + tag + '?project=' + projectId
          ...
          req = $.ajax
            url:url
          req.done (e) ->
            console.log 'existing tag' if e.length
            console.log 'new tag' if !e.length
            addTagAjax = ->
              $.ajax
                url:'/v1/tags/' + tag + '?project=' + projectId
                type:'post'
...
formDataObj = -># {{{
  ...
  url = '/v1/projects/' + id + '/' + title + '?description=' + description + '&short=' + summary + '&launched=' + launch + '&featured=' + featured + '&sticky=' + sticky + '&inLab=' + labs
  $.ajax# {{{
    url:url
    type:'put'
    success: (d) ->
      ...
      createTagsObj = ->
        # rewritten id assignment
        tagArr = $('#Project_tags').val()
        console.log 'tagArr'
        console.log tagArr
        $.each tagArr, (i,l) ->
          tag = l
...
$('#project-form').submit ->
    addEdit = $(this).attr('class')
    console.log addEdit
    postNewProject() if addEdit == 'add'
    formDataObj() if addEdit != 'add'
    #formDataObj()
...
$ ->
  ...
  fetchServiceCat = ->
    ...
    req.done (e) ->
      dataArr = {category2:[]}
      $(e).each ->
        dataArr.category2.push(this)
        source = $('#service-category-list').html()
        template = Handlebars.compile(source)
        data = dataArr
        $('#service-cat-form-elm').html(template(data))

  fetchServiceCat() if $('#project-form').length # }}}
  ...
  fetchServiceCatList = ->
    ...
    req.done (e) ->
      $(e).each ->
        listing = '<li>' + this.category + '</li>'
        $('#list-serviceCat').append(listing)
  fetchServiceCatList() if $('#list-serviceCat').length# }}}

  addServiceCat = ->
    $('#serviceCat-form').submit ->
      newCat = $('#Project_title').val()
      req = $.ajax
        url:'/v1/serviceCat/' + newCat
        type:'post'
      req.done (e) ->
        console.log e
        window.location = '/listServiceCat'
  addServiceCat() if $('#serviceCat-form').length
  ...
  workCat = ->
    fetchWorkCat = -># {{{
      ...
      req.done (e) ->
        dataArr = {category1:[]}
        $(e).each ->
          dataArr.category1.push(this)
          source = $('#work-category-list').html()
          template = Handlebars.compile(source)
          data = dataArr
          $('#work-cat-form-elm').html(template(data))
    fetchWorkCat() if $('#project-form').length
    ...
    fetchWorkCatList = ->
      ...
      req.done (e) ->
        $(e).each ->
          listing = '<li>' + this.category + '</li>'
          $('#list-workCat').append(listing)
    fetchWorkCatList() if $('#list-workCat').length

    addWorkCat = ->
      $('#workCat-form').submit ->
        newCat = $('#Project_title').val()
        req = $.ajax
          url:'/v1/workCat/' + newCat
          type:'post'
        req.done (e) ->
          console.log e
          window.location = '/listWorkCat'
    addWorkCat() if $('#workCat-form').length
...
# {{{
    ...
    $('#project-form').addClass('add') if typeof id == 'undefined'
    url = '/v1/projects/' + id
    $.ajax# {{{
      url:url
      dataType:'json'
      success: (d) ->
        ...
        console.log 'tags'
        console.log d
        populateWorkServiceCategories = ->
          console.log 'populate work and service'
          req = $.ajax
            url:'/v1/workCat'
            success: (e) ->
              console.log e
        ...
        console.log d.featured
        #$('#Project_tags').val(d.featured).tagsInput()
        $('#Project_tags').tagsInput()
        $('#Project_tags_tagsinput').remove()
